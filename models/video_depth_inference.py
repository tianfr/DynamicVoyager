...
class DepthCrafterDemo:
    ...
    def infer(
        ...
        # res = (res - res.min()) / (res.max() - res.min()).cpu().numpy()
        # visualize the depth map and save the results
        # vis = vis_sequence_depth(res)
        # save the depth map and visualization with the target FPS
        # save_path = os.path.join(
            # save_folder, os.path.splitext(os.path.basename("test.mp4"))[0]
        # )
        # os.makedirs(os.path.dirname(save_path), exist_ok=True)
        # save_video(res, save_path + "_depth.mp4", fps=target_fps)
        # save_video(vis, save_path + "_vis.mp4", fps=target_fps)
        # save_video(video.cpu().numpy(), save_path + "_input.mp4", fps=target_fps)
        # if save_npz:
...
def estimate_depth(
    ...
    # video_depth = video_depth.permute(0, 3, 1, 2)
    
    clear_all_gpu_variables(depthcrafter_demo)
    gc.collect()
    torch.cuda.empty_cache()
