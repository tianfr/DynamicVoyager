...
def video2images(video_path):
    ...
    while True:
        ...
        video_frames.append(ToTensor()(Image.fromarray(cropped_frame.astype(np.uint8)).convert("RGB")))
...
def video2images_new(video_path):
    """
    Extracts frames from a video and saves them as images.
...
def prepare_scheduler(scheduler):
    ...
    # if hasattr(scheduler.config, "original_inference_steps"):
    #     new_config = dict(scheduler.config)
    #     del new_config["original_inference_steps"]
    #     scheduler._internal_dict = FrozenDict(new_config)
    return scheduler
...
def empty_cache(device="cuda:0"):
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats(device)
    torch.cuda.reset_accumulated_memory_stats(device)
    gc.collect()

def clear_all_gpu_variables(obj, visited=None, depth=0):
    """
    递归清理类或嵌套对象中的所有 GPU 张量变量。

    :param obj: 要清理的对象
    :param visited: 用于记录已访问对象，防止重复清理
    """
    if visited is None:
        visited = set()
    ...
    for attr_name in dir(obj):
        ...
        # 检查是否是 GPU 张量
        if isinstance(attr_value, torch.Tensor) and attr_value.device.type == "cuda":
            print(f"Clearing GPU tensor: {attr_name}")
            ...
            for i, item in enumerate(attr_value):
                if isinstance(item, torch.Tensor) and item.device.type == "cuda":
                    flag = True
                    print(f"Clearing GPU tensor: {attr_name}[{i}]")
                    attr_value[i] = None
            if flag:
                delattr(obj, attr_name)
            ...
            for key, item in attr_value.items():
                if isinstance(item, torch.Tensor) and item.device.type == "cuda":
                    print(f"Clearing GPU tensor: {attr_name}[{key}]")
                    attr_value[key] = None
            if flag:
                delattr(obj, attr_name)
        elif hasattr(attr_value, "device") and depth < 2:
            # try:
            #     attr_value = attr_value.to("cpu")
            #     delattr(obj, attr_name)
            # except Exception as e:
                # pass
            clear_all_gpu_variables(attr_value, visited, depth+1)
    torch.cuda.empty_cache()